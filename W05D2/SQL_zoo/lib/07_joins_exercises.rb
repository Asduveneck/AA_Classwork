...
def films_and_stars_from_sixty_two
  ...
  execute(<<-SQL)
  SELECT
    movies.title, actors.name AS lead_star
  FROM
    movies
  JOIN
    castings ON movies.id = castings.movie_id
...
def travoltas_busiest_years
  ...
  execute(<<-SQL)
  SELECT
    movies.yr AS year, COUNT(*) AS movies_made
  FROM
    movies
  JOIN 
    castings ON movies.id = castings.movie_id
...
def andrews_films_and_leads
  ...
  execute(<<-SQL)
  SELECT
    movies.title AS title, actors.name AS leading_actor
  FROM
    movies 
  JOIN 
    castings ON movies.id = castings.movie_id
...
def prolific_actors
  ...
  execute(<<-SQL) 
  SELECT
    actors.name AS prolific_actors
  FROM 
    actors 
  JOIN
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
  SELECT
    DISTINCT actors.name AS actors
  FROM
    movies
  JOIN
    castings on movies.id = castings.movie_id 
